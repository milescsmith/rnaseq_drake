...
Samples with a PC1 zscore > 3 (those that fall beyond the red line below) are dropped from analysis.
...
if (length(removed_outliers) > 0) {
  message(str_glue("Removing the following samples: {removed_outliers}"))
}
```
...
down_table %>%
  ...
  rename(pvalue = paste0("pvalue", footnote_marker_symbol(1)),
         stat = paste0("stat", footnote_marker_symbol(2)),
         padj = paste0("padj", footnote_marker_symbol(3)),
...
t(vsd_exprs)[,rownames(module_ISGs)] %>%
    pheatmap(scale = 'column',
             border_color = NA,
             fontsize_row = 6, 
             fontsize_col = 6, 
...
vsd_exprs %>% 
  ...
  data.table::fwrite(row.names = TRUE,
                     file = "filtered_normalized_stabilized_expression.csv")
...
counts(dds_with_scores) %>%
  as.data.frame() %>%
  data.table::fwrite(row.names = TRUE,
                     file = "filtered_transcript_counts.csv")
res %>%
  as.data.frame() %>%
  data.table::fwrite(row.names = TRUE,
