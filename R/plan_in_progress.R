analysis_plan = drake_plan(
  ...
  advanta_plate_1 <-
    read_excel(
      path = "metadata/final_library_samples_for_omrf_(from_fludigm).xlsx",
      sheet = "sample Plate 1",
      skip = 21
    ) %>%
    clean_names() %>%
    rename(
      plate_id = "x96_well_plate_id",
      concentration = "concentration_ng_m_l",
      pool_id = "pool_id_by_fldm"
    ) %>%
    select(
      sample_name,
      plate_id,
  ...
  advanta_plate_info =
    ...
    mutate(
      plate_id = as_factor(plate_id),
      sample_description = as_factor(sample_description),
      ifc = as_factor(ifc)
    )
  ...
  tx_files =
    ...
    purrr::discard(.p = is.na(match(names(.), study_metadata$sample_name)))
  ...
  final_md = study_metadata %>%
    ...
    left_join(advanta_plate_info) %>%
    rename(cell_type = sample_description) %>%
  ...
  counts = tximport(pruned_samples,
                    type = "salmon",
                    txIn = TRUE,
                    txOut = FALSE,
                    tx2gene = annot,
  ...
  banchereau_module_scores =
    tirosh_score_modules(expr_obj = vsd_exprs, module_list = banchereau_modules) %>%
  ...
  annotated_modules = module_annotation %>%
  ...
  module_tbl = c(banchereau_modules, ldg_modules, metasignature_module) %>%
    enframe() %>%
    unnest(cols = "value") %>%
    rename(module = name,
           gene = value) %>%
    inner_join(as_tibble(module_annotation,
                         rownames = "module"))
